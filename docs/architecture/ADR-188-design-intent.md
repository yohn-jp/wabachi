...
- `src/design/contracts.ts` owns transport-neutral Design Change, review, Implementation linkage, certification evidence, and lifecycle wire types. Semantic Change Set content is separate from lifecycle/evidence records. These types do not grant Inari authorization or Nawabari execution permission.
...
Source-level Code Intent remains inside Canon at `src/architecture/canon/code-intent-contract.ts`. It attaches purpose, responsibility, invariant, and prohibition statements to existing repository mappings. It reuses Canon repository-mapping types and does not turn every source symbol into an Architecture Canon element.
...
- Review, external Implementation linkage, certification, and lifecycle records reference the Change Set digest/revision rather than copying Inari authorization fields.
