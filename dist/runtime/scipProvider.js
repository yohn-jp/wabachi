 ...
 */
export function createScipTypescriptProvider() {
    return {
...
function toObservations(index, context) {
    ...
    for (const document of index.documents) {
        ...
        for (const symbolInfo of document.symbols) {
            const definitionOccurrence = document.occurrences.find((occurrence) => occurrence.symbol === symbolInfo.symbol && (occurrence.symbol_roles & DEFINITION_ROLE) === DEFINITION_ROLE);
            ...
            for (const relationship of symbolInfo.relationships) {
                const predicate = relationship.is_implementation
                    ? "implements"
...
function toSpan(range) {
    ...
    const [line, startCharacter, endCharacter] = range;
    return `${line}:${startCharacter}-${line}:${endCharacter}`;
}
//# sourceMappingURL=scipProvider.js.map