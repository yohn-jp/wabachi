 ...
 * common envelope.
 */
const RELATION_TO_PREDICATE: Readonly<Record<string, ObservationPredicate>> = {
  calls: "calls",
  imports: "imports",
  extends: "extends",
...
export function createGraftProvider(options: GraftProviderOptions = {}): Provider {
  const binary = options.binary ?? GRAFT_BINARY;
  let resolvedVersion = "unknown";

  return {
    get identity() {
      return { id: "graft", version: resolvedVersion, determinism: "deterministic" as const };
    },

    async isAvailable(_context: ProviderContext): Promise<boolean> {
      try {
    ...
    async execute(context: ProviderContext): Promise<ProviderExecutionResult> {
      ...
      await writeFile(
        path.join(context.artifactRoot, invocationRelativePath),
      ...
      const wiring = JSON.parse(
        await readFile(path.join(context.artifactRoot, wiringRelativePath), "utf8"),
      ) as GraftWiringGraph;

      const identity = { id: "graft", version, determinism: "deterministic" as const };
 ...
 */
function adaptWiringGraph(
  wiring: GraftWiringGraph,
  provider: { id: string; version: string; determinism: "deterministic" },
  context: ProviderContext,
): Observation[] {
  const nodesById = new Map<string, GraftWiringNode>(wiring.nodes.map((node) => [node.id, node]));
...
function toObservation(
  edge: GraftWiringEdge,
  sourceNode: GraftWiringNode,
  targetNode: GraftWiringNode,
